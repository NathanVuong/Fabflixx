...
public class GetGenresServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            // Get top 20 movies
            String allGenresQuery = "SELECT DISTINCT name FROM genres ORDER BY name ASC;";
            ResultSet allGenres = statementOne.executeQuery(allGenresQuery);
            JsonArray jsonArray = new JsonArray();

            while (allGenres.next()) {
                String genre = allGenres.getString("name");
                jsonArray.add(genre);
            }
            allGenres.close();
            statementOne.close();
